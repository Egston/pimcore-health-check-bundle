...
**Security caveat — `SUB_REQUEST` skips Symfony firewalls**: `HttpKernelInterface::SUB_REQUEST` deliberately bypasses Symfony's `FirewallListener` and `access_control` rules — that's how the kernel implements forward-style internal dispatch. As a result, a `graphql_internal` probe against `/admin/...` or `/api/...` would reach the controller with whatever security token the parent request had (none, in a kubelet probe). Configure `path:` ONLY for endpoints intended to be world-accessible (e.g. the DataHub `public-content` endpoint, which authenticates via the `apikey` query param rather than firewall). Never aim `graphql_internal` at routes that depend on firewall enforcement to gate sensitive data or mutations. As a misconfiguration guard, the bundle rejects `path:` values that resolve to the `/admin` prefix at container-compile time — that's the one firewall-protected prefix every Pimcore deployment shares. Other host-specific firewall prefixes (e.g. JWT-bundle's `/api`, WebDAV's `/asset/webdav`, custom firewalls) are out of scope for this guard; the host's `security.yaml` is the source of truth, and operator review of `path:` against it carries the rest.

**Recommended probe wiring**: with `graphql_internal` configured, the kubelet probe answering `/readyz` (`path:` above) belongs on the **PHP-FPM container**, dispatched via `cgi-fcgi` exec — that places restart authority on the pod that actually hosts the application logic.
